(() => {
  ...
  async function showExternalPlaylistDetail(playlistId, meta) {
    ...
    if (!tracks || !tracks.length) {
      tracksContainer.innerHTML = `<div class="empty-state"><p>Could not load playlist tracks.</p></div>`;
      return;
    }

    heroName.textContent = meta?.name || 'Playlist';
    const parts = [];
    if (meta?.author) parts.push(escapeHtml(meta.author));
    parts.push(`${tracks.length} song${tracks.length !== 1 ? 's' : ''}`);
    heroMeta.innerHTML = parts.join(' \u00B7 ');
  ...
  function showPlaylistContextMenu(e, playlistId, meta) {
    ...
    menu.addEventListener('click', async (ev) => {
      ...
      switch (item.dataset.action) {
        ...
        case 'share':
          navigator.clipboard.writeText(`https://music.youtube.com/playlist?list=${playlistId}`);
          showToast('Link copied to clipboard');
          break;
      }
      removeContextMenu();
